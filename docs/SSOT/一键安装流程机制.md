...
Phase0/Phase1/Phase2 最小门禁（固定顺序）
- 固定顺序（最小纵向切片，必须明确且可审计）：
  安装 → 配置 → 验收 → 回滚/恢复 → 证据落盘

Phase0（Baseline & Reality Sync）
- 最小门禁：
  - Reality Snapshot 已生成，(1)(2) 硬证据一致。
  - Files 白名单与回滚策略已写入 SPEC。
- 最小 DoD：
  - 已形成可执行任务输入（目标、范围、风险、白名单、回滚）且可被审计复现。

Phase1（Implementation & Verification）
- 最小门禁：
  - 严格按 Files 白名单实施；不得越界。
  - 必须完成“安装 → 配置 → 验收 → 回滚/恢复演练（或可验证回滚路径）→ 证据落盘”闭环。
- 最小 DoD：
  - 相关 checks/本地验证命令达到 PASS。
  - Evidence Pack 完整，且能绑定到 PR/commit/checks。

Phase2（Audit & Closure）
- 最小门禁：
  - 审计 PASS + Done Gate 满足。
  - 合并后状态可复核（post-merge 证据可取）。
- 最小 DoD：
  - 任务具备可回滚性（revert 可执行）。
  - 状态更新与后续项（如有）可追溯，不留“口头完成”。

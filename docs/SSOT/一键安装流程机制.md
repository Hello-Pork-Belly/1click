...
### Phase2: Expansion
- Entry Criteria
  - Phase1 Done（最小纵向切片闭环已达成）。
- DoD
  - 扩展模块/recipes/多拓扑/性能与加固，但仍必须满足 evidence + DoD 闭环。
- Evidence
  - Evidence MUST 落盘到 `docs/SSOT/EVIDENCE/`。
  - 文件命名 MUST 为 `<captured_at_utc>_<mode>_<main_sha>_<topic>.md`。
  - 哨兵 MAY 通过 SHA-pinned raw evidence 文件执行 Evidence-by-file 审核。
- Stop/Go
  - routine：缺项 => UNKNOWN + Evidence Gaps（不 STOP）。
  - milestone：P0/BLOCKED 条件沿用既有红线（本节不改写）。
...
#### PHASES.yml relationship (clarification)

- `docs/PHASES.yml` is the single truth for **repository lifecycle phases** (e.g. PLAN / IMPLEMENT / VERIFY / RELEASE).
- Playbook `Phase0/Phase1/Phase2` are **execution gate phases** used to sequence work and DoD closure (governance/gates), and are not lifecycle states.
- Non-conflict rule: if wording ever appears to conflict, **`docs/PHASES.yml` wins** as lifecycle truth; Playbook phases only explain execution order/gates.
- Rough mapping example (non-binding):
  - Phase0 (Governance/Foundation Ready) ≈ PLAN / early IMPLEMENT
  - Phase1 (Minimum Vertical Slice) ≈ IMPLEMENT / VERIFY
  - Phase2 (Expansion) ≈ VERIFY / RELEASE hardening

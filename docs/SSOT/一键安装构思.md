...
- 审计模式（SENTINEL v2）：默认 `mode: routine`（只采证据与记录，不追 A0）；只有 Commander 明确声明 `mode: milestone` 才做 P0 对齐审计（要求 A0 `post_merge_main_head/main_head` 与硬证据闭环一致，并补齐 merged facts）。

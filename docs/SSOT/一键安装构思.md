...
- 单一事实源（SSOT）：以仓库 `docs/SSOT/` 为准；任何结论必须引用 SHA-pinned raw 的对应文件；`raw(main)` 仅可用于导航/发现，不得作为真值校验形式。
...
（边界声明，Round1）
- 本文件仅承载蓝图层内容：目标、范围、约束、术语、质量条款与架构方向。
- 执行流程/门禁步骤/Stop-Go/DoD 细则统一归位到《docs/SSOT/一键安装流程机制.md》。
- 入口/目录/模板/接手与证据落盘位置统一归位到《docs/SSOT/1click核心执行框架 + 必要文档.md》。
- 本文件不再承载 PR/gh 操作步骤、Evidence Pack 字段枚举、角色审计流程细节。

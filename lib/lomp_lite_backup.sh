...
lomp_backup_host() {
  ...
  lomp_log_info "lomp-lite host backup wrote ${host_out}"
}

lomp_find_hub_dump_source() {
  rootfs=${1:-}

  if [ -n "${rootfs}" ] && [ "${rootfs}" != "/" ]; then
    for candidate in \
      "${rootfs}/var/lib/lomp-lite/mariadb.sql" \
      "${rootfs}/var/backups/lomp-lite/mariadb.sql" \
...
lomp_backup_hub() {
  ...
  mkdir -p "${hub_out}"
  dump_source=$(lomp_find_hub_dump_source "${rootfs}") || {
  ...
  lomp_log_info "lomp-lite hub backup wrote ${dump_target}"
}

lomp_lite_backup() {
  case "${LOMP_ROLE}" in
    host) lomp_backup_host ;;
    hub) lomp_backup_hub ;;
  esac
}

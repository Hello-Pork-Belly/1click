...
lnmp_standard_find_dump_source() {
  rootfs=${1:-}

  if [ -n "${rootfs}" ] && [ "${rootfs}" != "/" ]; then
    for candidate in \
      "${rootfs}/var/lib/lnmp-standard/mariadb.sql" \
      "${rootfs}/var/backups/lnmp-standard/mariadb.sql" \
...
lnmp_standard_backup() {
  rootfs=${LNMP_STANDARD_ROOTFS:-}
  [ -n "${rootfs}" ] || rootfs=/
  out_dir=${LNMP_STANDARD_OUT}
  ...
  dump_source=$(lnmp_standard_find_dump_source "${rootfs}") || {
  ...
  lomp_log_info "lnmp-standard backup wrote ${wp_out} and ${db_out}"
}

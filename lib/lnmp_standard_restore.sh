...
lnmp_standard_restore() {
  rootfs=${LNMP_STANDARD_ROOTFS:-}
  [ -n "${rootfs}" ] || rootfs=/
  from_dir=${LNMP_STANDARD_FROM}
  ...
  if [ "${rootfs}" != "/" ]; then
    db_dest="${rootfs}/var/lib/lnmp-standard/mariadb.sql"
  ...
  lomp_log_info "lnmp-standard restore wrote ${wp_dest} and ${db_dest}"
  lomp_log_info "lnmp-standard restore: Redis is cache-only and is not restored by design"
}

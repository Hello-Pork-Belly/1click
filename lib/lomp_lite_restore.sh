...
lomp_restore_host() {
  ...
  lomp_log_info "lomp-lite host restore wrote ${restore_dest}"
}
...
lomp_restore_hub() {
  ...
  if [ "${rootfs}" != "/" ]; then
    restore_dest="${rootfs}/var/lib/lomp-lite/mariadb.sql"
  ...
  lomp_log_info "lomp-lite hub restore wrote ${restore_dest}"
  lomp_log_info "lomp-lite hub restore: Redis is cache-only and is not restored by design"
}

lomp_lite_restore() {
  case "${LOMP_ROLE}" in
    host) lomp_restore_host ;;
    hub) lomp_restore_hub ;;
  esac
}
